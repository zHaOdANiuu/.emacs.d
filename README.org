...
- *Programming.* Lang modes cover C/C++, JavaScript and TypeScript, web templates,
  powershell, Markdown, JSON, and YAML.  Eglot provides LSP navigation, renaming,
  references, and code actions; Flymake shows diagnostics; Dape and GDB handle
  debugging.
- *Projects and navigation.* Built-in Project support with ripgrep search,
  xref, Citre/readtags, bookmarks, and Dired makes it easy to find and manage
  code and files.
- *Terminals and daily tools.* Ghostel provides a project-aware terminal,
  while Eshell, EWW, translation, mail/news, and IRC cover common work
